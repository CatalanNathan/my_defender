...
void create_wall(char *src_image, V2f pos, var_t *var, char *tag)
{
    ...
    set_shape_obj(&var->wall.button, (sfIntRect){0, 0, get_size_obj(&var->
    wall.button).x / 2, get_size_obj(&var->wall.button).y}, true);
...
void create_tower(char *src_img, V2f pos, var_t *var, char *tag)
{
    ...
    set_shape_obj(&var->tower.tower, (sfIntRect){150,
    0, 120, get_size_obj(&var->tower.tower).y}, true);
