...
void display_engine(engine_t *engine)
{
    ...
    if (engine->win_settings.background.display == true && engine->win_settings.background.exist == true)
        sfRenderWindow_drawSprite(engine->win_settings.engine, engine->win_settings.background.sprite, NULL);
    while (order != 10 && engine->game.list != NULL) {
        if (engine->game.list->settings.display == true && engine->game.list->settings.exist == true && engine->game.list->settings.display_order == order)
            sfRenderWindow_drawSprite(engine->win_settings.engine, engine->game.list->settings.sprite, NULL);
