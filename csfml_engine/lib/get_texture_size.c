...
sfVector2u get_texture_size_tag(char *tag, engine_t *engine)
{
    ...
    while (engine->game.list != NULL) {
        if (equal(engine->game.list->settings.tag, tag) == true) {
            size.x = sfTexture_getSize(engine->game.list->settings.texture).x * sfSprite_getScale(engine->game.list->settings.sprite).x;
            size.y = sfTexture_getSize(engine->game.list->settings.texture).y * sfSprite_getScale(engine->game.list->settings.sprite).y;
