...
void shape_selected_col(engine_t *engine)
{
...
sfVector2u get_size_col(engine_t *engine)
{
    shape_selected_col(engine);
    return engine->game.list->settings.size;
}

bool check_collision(sfVector2f pos_1, sfVector2u size_1, engine_t *engine)
{
    sfVector2f pos_2 = sfSprite_getPosition(engine->game.list->settings.sprite);
    sfVector2u size_2 = get_size_col(engine);
...
bool get_collision_tag(char *obj_tag, char *obstacles_tag, engine_t *engine)
{
    ...
    while (engine->game.list != NULL) {
        if (equal(engine->game.list->settings.tag, obstacles_tag) == true) {
            if (check_collision(pos_1, size_1, engine) == true) {
